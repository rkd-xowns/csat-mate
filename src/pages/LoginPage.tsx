...
export function LoginPage({ setSignupUsername }: LoginPageProps) {
  ...
  // --- 나머지 기능 로직: 소셜 로그인 관련 함수 복원 및 추가 ---
  useEffect(() => {
    if (error) {
  ...
  return (
    <div className="page-layout login-container justify-center">
      <div className="login-content-area max-w-lg flex flex-col items-center">
        ...
        {error && <p className="text-red-500 text-sm text-center -my-2 animate-pulse">{error}</p>}
        ...
        <div className="actions-panel w-full flex flex-col" style={{ gap: `${adaptiveSize.formInternalGap}px` }}>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input id="email" type="email" placeholder="아이디를 입력하세요" value={email} onChange={e => setEmail(e.target.value)} className="login-input" onFocus={() => setFocusState('email')} onBlur={() => setFocusState('none')} disabled={isLoading} />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="password-wrapper items-stretch">
              <div className="relative w-full">
                <input id="password" type={showPassword ? 'text' : 'password'} placeholder="비밀번호를 입력하세요" value={password} onChange={e => setPassword(e.target.value)} className="login-input pr-10" onFocus={() => setFocusState('password')} onBlur={() => setFocusState('none')} disabled={isLoading}/>
              ...
              {/* [변경] 비밀번호 찾기 링크 스타일 수정 */}
              <button 
                onClick={() => navigate('/forgot-password')} 
                className="forgot-password-link text-right w-full mt-1 underline transition-opacity opacity-70 hover:opacity-100" 
                style={{ 
                  fontSize: `${adaptiveSize.linkFontSize}px`,
